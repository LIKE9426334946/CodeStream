...
- 学习显示页（手机和电脑）：目录切换、代码流搜索、步骤式阅读、代码横向滚动、合并查看；缓存仅用于快速显示和请求失败时保留内容，每次打开、切回页面或恢复网络都会请求服务器最新数据，也可手动刷新。电脑阅读代码流时仍显示刷新按钮。
- 代码框右上角提供 `Copy` 按钮，复制正文并保留换行和缩进，不包含步骤编号与说明标签；`View` 合并模式可复制整个代码框。复制成功短暂显示 `Copied`，兼容 HTTP 页面。
- 电脑管理页：固定账号登录；新增、修改、删除目录和代码流；添加代码或说明步骤；三级拖拽排序；合并预览全部内容并一键复制。
